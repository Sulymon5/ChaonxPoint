...
public class RealmBackupRestore {
    ...
    public static boolean restore(Activity mActivity) {
        ...
        if (copyBundledRealmFile(restoreFilePath, mActivity.getString(R.string.app_name) + PreferenceManager.getToken(mActivity) + ".realm", mActivity)) {
            file.delete();
            return true;
        } else {
            return false;
    ...
    public static void deleteData(Activity activity) {
        ...
        try {
            ChatonXApplication.DeleteRealmDatabaseInstance();
